...
export class LivraisonComponent implements OnInit {
  ...
  // Pagination commandes dans select
  ...
  validate(): boolean {
    this.errors = {};
    if (!this.form.numeroSuivi || this.form.numeroSuivi.trim() === '')
      this.errors['numeroSuivi'] = 'Le numéro de suivi est obligatoire';
    if (!this.form.transporteur || this.form.transporteur.trim() === '')
      this.errors['transporteur'] = 'Le transporteur est obligatoire';
    if (!this.form.dateExpedition)
      this.errors['dateExpedition'] = 'La date d\'expédition est obligatoire';
    if (!this.form.statut)
  ...
  save() {
    ...
    if (this.editMode) {
      this.ds.updateLivraisonObs(this.form).subscribe({
        next: () => { this.load(); this.closeModal(); },
        error: (err) => console.error(err)
      });
    } else {
      this.ds.addLivraisonObs(this.form).subscribe({
        next: () => { this.load(); this.closeModal(); },
        error: (err) => console.error(err)
      });
    }
